...
⚠️ **NOTE**: Ce fichier contient TOUTES les missions. Regardez votre branche actuelle pour savoir laquelle faire !
...
### ✅ Critères de validation
- [ ] Guard `isStopDTO` pour valider les arrêts
- [ ] Guard `isPlannedDepartureDTO` pour les départs planifiés
- [ ] Guard `isDelayInfoDTO` pour les informations de retard
- [ ] Fonction `parseStops` qui parse et valide un tableau d'arrêts
- [ ] Fonction `parsePlannedDepartures` avec validation complète
